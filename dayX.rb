...
class Test_Day_10 < Minitest::Test
  ...
  def test_calculate_part3
    test_case = 1
    assert_equal(test_case, Main.new(File.open('./input/dayX-input.txt').readlines).calculate_part1)
  end

  def test_calculate_part4
    test_case = 2
    assert_equal(test_case, Main.new(File.open('./input/dayX-input.txt').readlines).calculate_part2)
  end
end
